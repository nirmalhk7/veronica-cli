...
def list_birthdays(google_setup):
    ...
    birthday_arr=[]
    d= service.people().connections().list(resourceName="people/me",personFields="birthdays,names").execute()
    birthday_arr+= d["connections"]
    while "nextPageToken" in d:
        d=service.people().connections().list(resourceName="people/me",personFields="birthdays,names",pageToken=d["nextPageToken"]).execute()
        birthday_arr+= d["connections"]
    events=[]
    for contact in birthday_arr:
        if("birthdays" in contact):
            event= EventInterface(
                link="https://contacts.google.com/person/"+contact["resourceName"].split("/")[1],
        ...
        if (calendar.get("selected", False) == True):
            ...
            for cal_event in cal_events:
                event= EventInterface(
                    ...
                    calendar.get("summaryOverride", None) or calendar.get("summary", None),
