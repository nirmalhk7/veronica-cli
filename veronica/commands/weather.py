...
def do_weather(self, args):
    ...
    with Progress(transient=True) as progress:
        t1 = progress.add_task("[blue]Retrieving coordinates for {} ...".format(
            args.capitalize() if args else "your current location"), start=False)
        if (args != ""):
            g = geocoder.osm(args)
        elif (args == ""):
            g = geocoder.ip('me')
        ...
        if(self.settings["env"]["WEATHER_DARKSKY"]):
            response = requests.get('https://api.darksky.net/forecast/' +
                                    self.settings["env"]["WEATHER_DARKSKY"] +
                                    '/' +
                                    str(coord[0]) +
                                    ',' +
                                    str(coord[1]))
    if(response and response.status_code == 200):
        data = response.json()
        res = data['currently']['summary'] + " today with " + \
            f_to_c(data['currently']['temperature']) + "° C."
        if(data['currently']['precipProbability'] > 0.5):
            res += " Expect a " + str(data['currently']['precipProbability'] * 100) + \
